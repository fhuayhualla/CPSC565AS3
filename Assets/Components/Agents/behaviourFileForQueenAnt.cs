...
public class behaviourFileForQueenAnt : MonoBehaviour
{
    ...
    private float maximumHealth = 100f; // Variable for maximum health.
    public float health = 100f; // The current health of the ant.
    public float healthDecayRate = 1f; // The rate at which health decreases over time.
    ...
    void Update()
    {
        ...
        if (canMove && !isClimbing)
        {
            ...
            StartCoroutine(ProduceNestBlock());
    ...
    // Determines whether the queen ant should attempt to climb. 
    bool ShouldClimb()
    {
        Vector3 positionInFront = transform.position + transform.forward;
        AbstractBlock blockInFront = WorldManager.Instance.GetBlock(Mathf.FloorToInt(positionInFront.x), Mathf.FloorToInt(positionInFront.y), Mathf.FloorToInt(positionInFront.z));
        ...
        // Check if the block in front is not climbable and the block above is climbable
    ...
    void ConsumeMulchBlock()
    {
        ...
        if (IsValidPosition(positionBelow))
        {
            ...
            if (!otherAntOnBlock)
            {
                ...
                WorldManager.Instance.SetBlock(x, y, z, new AirBlock()); // Replace Mulch with Air block
            }
            else
            {
    ...
    void SeekNewDirectionOrWait()
    {
        ...
        {
            // Employing waiting as a means to help queen ant get unstuck.
    ...
    IEnumerator ProduceNestBlock()
    {
        // Wait for 2 seconds before producing the nest block.
        yield return new WaitForSeconds(2);

        // Check if we have more than 1/3 of our maximum health.
        if (health > maximumHealth / 3)
        {
            ...
            Vector3 positionBelow = transform.position - Vector3.up; 
            int x = Mathf.FloorToInt(positionBelow.x);
            int y = Mathf.FloorToInt(positionBelow.y);
            int z = Mathf.FloorToInt(positionBelow.z);

            if (WorldManager.Instance.IsValidPosition(x, y, z))
            {
                WorldManager.Instance.SetBlock(x, y, z, new NestBlock()); // Assuming NestBlock is a valid block type
                
                Debug.Log("Nest block produced at (" + x + ", " + y + ", " + z + "). Current health: " + health);
            }
            else
            {
                Debug.Log("Invalid position for nest block. Current health: " + health);
            }
        }
        else
        {
            Debug.Log("Not enough health to produce a nest block. Current health: " + health);
        }
    }
